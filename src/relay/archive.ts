...
export class InMemorySessionArchiveStore {
  ...
  mergeActiveRecordings(fromSessionId: string, toSessionId: string): void {
    ...
    target.lastRecordedSequence = Math.max(
      target.lastRecordedSequence ?? 0,
      source.lastRecordedSequence ?? 0
    );
