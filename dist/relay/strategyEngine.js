...
function recommendationScore(label, signals) {
...
function computeConfidenceScore(scores) {
...
function nearestThresholdDistance(value, thresholds) {
    if (value == null)
        return 100;
    let min = 100;
...
function computeStabilityScore(signals, previousSignals) {
    let score = 84;
    const distances = [
    ...
    if (boundaryDistance < 4)
        score -= 32;
    else if (boundaryDistance < 8)
        score -= 18;
    else if (boundaryDistance < 14)
        score -= 10;
    if (previousSignals) {
...
function buildTrendReason(changed, current, previousSignals) {
    if (!previousSignals) {
    ...
    if (changed) {
        if (fuelDelta >= 10)
            return 'recommendation changed because fuel margin dropped faster than expected';
        if (tyreDelta >= 10)
            return 'recommendation changed because tyre degradation accelerated';
        if (trafficDelta >= 10)
            return 'recommendation changed because projected rejoin traffic increased';
        if (pitLossDelta >= 10)
            return 'recommendation changed because pit loss estimate increased';
    ...
    if (tyreDelta > 0)
        return 'recommendation held while tyre pressure increased toward pit window';
    if (fuelDelta > 0)
        return 'recommendation held while fuel risk increased but stayed manageable';
    if (trafficDelta > 0)
        return 'recommendation held while traffic risk increased, requiring close monitoring';
...
export class StrategyEngine {
    evaluate(input) {
        ...
        if (advanced.undercutScore != null && advanced.overcutScore != null) {
            ...
            else if (advanced.overcutScore >= 70 &&
        ...
        if (advanced.pitLossHeuristic != null &&
        ...
        if (advanced.compoundStintBias != null &&
        ...
        const rawRecommendationChanged = !!input.previousStrategy && input.previousStrategy.recommendation !== recommendation;
        ...
        const trendReason = buildTrendReason(recommendationChanged, signals, input.previousStrategy?.signals);
