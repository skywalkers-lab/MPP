...
현재 단계는 **Strategy Engine v1 (9단계)** 입니다.
...
이번 단계의 핵심은 계정 시스템 없이도 세션 문맥에서
사람 메모(판단 기록)와 기계 추천(룰 기반 전략)을 함께 볼 수 있게 하는 것입니다.
즉, 운영 이벤트 + notes + strategy recommendation이 같은 세션 타임라인/상세 문맥에서 공존합니다.
...
- Strategy card에서 recommendation/severity/reasons 확인
...
- `GET /api/viewer/strategy/:sessionId`
  - 세션 스냅샷 기반 룰 전략 추천
  - 추천 가능: `recommendation`, `severity`, `reasons`, `signals`, `generatedAt`
...
- Strategy Engine v1은 설명 가능한 룰 기반 추천이며 Monte Carlo/ML 단계가 아님
- v2에서 undercut/overcut, traffic model, alternative strategy A/B 등 고도화 예정
- notes + ops timeline + strategy card 조합은 이후 전략 엔진 고도화의 기반
