...
npx esbuild src/desktop/launcher.ts \
  --bundle \
  --platform=node \
  --format=cjs \
  --target=node18 \
  --outfile=.pkgbuild/desktop-entry.cjs

cat > .pkgbuild/package.json <<'EOF'
{
  "name": "mpp-desktop-launcher",
  "version": "0.0.0",
  "type": "commonjs",
  "bin": "desktop-entry.cjs"
}
EOF

npx pkg .pkgbuild --targets node18-win-x64 --output release/MPP-Desktop.exe
cp release/MPP-Desktop.exe release/MPP-Setup.exe
cp release/MPP-Desktop.exe release/MPP-portable.exe

cat > release/README-MPP-Desktop.txt <<EOF
══════════════════════════════════════════════════
  MPP Desktop v${APP_VERSION}  —  Mission Pitwall Platform
...
echo ""
echo "═══════════════════════════════════════"
echo "  MPP Desktop Windows 패키징 완료"
echo "  버전: ${APP_VERSION}"
echo ""
echo "  출력 파일:"
echo "    release/MPP-Desktop.exe"
echo "    release/MPP-Setup.exe"
echo "    release/MPP-portable.exe"
echo "    release/README-MPP-Desktop.txt"
echo "═══════════════════════════════════════"
