...
- 권장 단일 실행 파일(.exe): https://github.com/skywalkers-lab/MPP/releases/latest/download/MPP-Desktop.exe
- 호환 자산(.exe): https://github.com/skywalkers-lab/MPP/releases/latest/download/MPP-Setup.exe
- 호환 자산(.exe): https://github.com/skywalkers-lab/MPP/releases/latest/download/MPP-portable.exe

주의: `Source code (zip/tar.gz)`는 설치 파일이 아닙니다. Windows 사용자는 반드시 `.exe` 자산을 받으세요.

Windows `.exe`를 실행하면 첫 실행에 Local Control Panel이 브라우저로 열립니다.
여기서 Render 배포 주소를 `Public Viewer URL`에 한 번 저장하면, `Relay WebSocket URL`은 자동으로 채워집니다.
Driver는 `Driver Sender 시작`으로 로컬 UDP 20777을 Render Relay로 업링크하고, Engineer는 같은 실행 파일에서 `Engineer 열기`로 Render Rooms만 바로 엽니다.
`Launch Mode`와 자동 시작을 저장해 두면 다음 실행부터 한 번에 동일 모드로 진입합니다.
게임에서 Telemetry UDP를 켜지 않으면 Driver 모드의 `UDP packets/10s`가 0으로 유지됩니다.
같은 팀이 같은 Room을 보려면 동일한 Render Relay 주소를 사용해야 합니다.
화면이 열리지 않으면 먼저 Local Control Panel에 저장된 `Public Viewer URL`이 올바른지 확인하세요.
그래도 접속이 안 되면 앱을 다시 실행하고, 로컬 보안 정책(방화벽/백신)에서 UDP 포트 접근이 차단되지 않았는지 확인하세요.
...
기본 동작에서 embedded UDP agent가 함께 시작됩니다.

- 기본 UDP 수신 포트: `20777`
- 기본 바인드 주소: `0.0.0.0`
- 비활성화: `MPP_EMBEDDED_AGENT=0`
