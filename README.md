...
현재 단계는 **Recording / Replay / Analysis (11단계)** 입니다.
...
이번 단계의 핵심은 live relay 세션을 종료 후 복기 가능한 아카이브 단위로 남기는 것입니다.
archive는 snapshot timeline + ops events + session notes + strategy 문맥을 함께 저장합니다.
여전히 보수적이고 설명 가능한 규칙 기반이며, 영상 재생/무거운 그래프 엔진/AI post-race coach는 범위 밖입니다.
...
- archive summary 목록 확인 (`sessionId`, 종료 시각, duration, snapshot/ops/note count)
- 세션별 archive summary 상세 확인
- unified timeline에서 `snapshot`, `ops_event`, `note`를 시간순으로 복기
- timeline의 snapshot 항목 선택 시 해당 시점의 player/session/strategy 요약 확인
